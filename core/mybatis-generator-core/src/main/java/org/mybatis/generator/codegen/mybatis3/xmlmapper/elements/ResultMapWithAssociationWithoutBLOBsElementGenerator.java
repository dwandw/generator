 ...
 * 
 */
public class ResultMapWithAssociationWithoutBLOBsElementGenerator extends AbstractXmlElementGenerator {

    private boolean isSimple;

    public ResultMapWithAssociationWithoutBLOBsElementGenerator(boolean isSimple) {
        super();
    ...
    public void addElements(XmlElement parentElement) {
        if (introspectedTable.getTableConfiguration().getAssociations().size() > 0) {
            ...
            answer.addAttribute(new Attribute("id", introspectedTable.getAssociationResultMapId()));
    ...
    private void addResultMapElements(XmlElement answer) {
        ...
        for (Association association : introspectedTable.getTableConfiguration().getAssociations()) {
            ...
            if (association.getType().equalsIgnoreCase("association")) {
                resultElement = new XmlElement("association");
            } else if (association.getType().equalsIgnoreCase("collection")) {
                resultElement = new XmlElement("collection");
            }
            resultElement
                    .addAttribute(new Attribute("property", getValidPropertyName(association.getIntrospectedTable().getFullyQualifiedTable().getDomainObjectName())));
            resultElement.addAttribute(new Attribute("resultMap", String.format("%s.%s", association.getIntrospectedTable().getMyBatis3SqlMapNamespace(), association
                    .getIntrospectedTable().getBaseResultMapId())));
            answer.addElement(resultElement);
        }
    }
