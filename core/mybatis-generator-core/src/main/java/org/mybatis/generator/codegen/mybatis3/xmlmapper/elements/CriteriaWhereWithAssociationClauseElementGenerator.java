 ...
 * 
 */
public class CriteriaWhereWithAssociationClauseElementGenerator extends AbstractXmlElementGenerator {

    public CriteriaWhereWithAssociationClauseElementGenerator() {
        super();
    }
    ...
    public void addElements(XmlElement parentElement) {
        if (introspectedTable.getTableConfiguration().getAssociations().size() > 0) {
            ...
            answer.addAttribute(new Attribute("id", introspectedTable.getCriteriaWhereWithAssociationClauseId()));
            ...
            trimElement.addElement(outerForEachElement);

            XmlElement ifElement = new XmlElement("if");
            ifElement.addAttribute(new Attribute("test", "criteria.valid"));
            outerForEachElement.addElement(ifElement);

            XmlElement _trimElement = new XmlElement("trim");
            _trimElement.addAttribute(new Attribute("prefix", "("));
            _trimElement.addAttribute(new Attribute("suffix", ")"));
            _trimElement.addAttribute(new Attribute("prefixOverrides", "and"));

            ifElement.addElement(_trimElement);

            _trimElement.addElement(getMiddleForEachElement(null));

            for (IntrospectedColumn introspectedColumn : introspectedTable.getNonBLOBColumns()) {
                if (stringHasValue(introspectedColumn.getTypeHandler())) {
                    _trimElement.addElement(getMiddleForEachElement(introspectedColumn));
                }
            }

            for (Association association : introspectedTable.getTableConfiguration().getAssociations()) {
                ifElement = new XmlElement("if");
                ifElement.addAttribute(new Attribute("test", String.format("%s != null", association.getIntrospectedTable().getCriteriaJavaProperty())));
                trimElement.addElement(ifElement);

                ifElement.addElement(new TextElement(" and "));
                outerForEachElement = new XmlElement("foreach");
                outerForEachElement.addAttribute(new Attribute("collection", String.format("%s.oredCriteria", association.getIntrospectedTable()
                        .getCriteriaJavaProperty())));
                outerForEachElement.addAttribute(new Attribute("item", "criteria"));
                outerForEachElement.addAttribute(new Attribute("separator", "or"));
                outerForEachElement.addAttribute(new Attribute("open", "("));
                outerForEachElement.addAttribute(new Attribute("close", ")"));
                ifElement.addElement(outerForEachElement);

                ifElement = new XmlElement("if");
                ifElement.addAttribute(new Attribute("test", "criteria.valid"));
                outerForEachElement.addElement(ifElement);

                _trimElement = new XmlElement("trim");
                _trimElement.addAttribute(new Attribute("prefix", "("));
                _trimElement.addAttribute(new Attribute("suffix", ")"));
                _trimElement.addAttribute(new Attribute("prefixOverrides", "and"));
